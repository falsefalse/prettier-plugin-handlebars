...
/* Handlebars' own rule, restated rather than imported from the printer: a backslash escapes the
 * enclosing quote and nothing else, so `\n` in a template is a backslash and an `n`. */
function literalValue(source: string): string | null {
  const quote = source[0];
  const quoted = (quote === '"' || quote === "'") && source.length >= 2 && source.endsWith(quote);

  return quoted ? source.slice(1, -1).split(`\\${quote}`).join(quote) : null;
}

/**
 * Every quoted literal's value, in document order. Which quote a literal wears is the printer's
 * to pick; what it holds is not. Comparing these across a format run is what says so - and it
 * catches the case rendering cannot, where the value never reaches the page.
 */
export function literalValues(program: Program): string[] {
  const values: string[] = [];

  const fromExpression = (expression: Expression): void => {
    const value = literalValue(expression.source);
    ...
    if (expression.type === 'SubExpression') {
      expressionsOf(expression).forEach(fromExpression);
    }
  };
  ...
  walk(program, (node) => {
    ...
    if (call) {
      expressionsOf(call).forEach(fromExpression);
    }

    for (const branch of node.type === 'BlockStatement' ? node.inverseChain ?? [] : []) {
      expressionsOf(branch).forEach(fromExpression);
    }
  });
